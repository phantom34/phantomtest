...
public class ImportBookPresenter implements ImportBookContract.Presenter {

    private ImportBookContract.View mView;


    @Inject
    public ImportBookPresenter(ImportBookContract.View view) {
        this.mView = view;
    }
    ...
    private void searchBook(ObservableEmitter<File> e, File parentFile) {
        if (null != parentFile && parentFile.listFiles().length > 0) {
            ...
            for (int i = 0; i < childFiles.length; i++) {
                if (childFiles[i].isFile() && childFiles[i].getName().substring(childFiles[i].getName().lastIndexOf("" +
                        ".") + 1).equalsIgnoreCase("txt") && childFiles[i].length() > 100 * 1024) {
                    //100kb
