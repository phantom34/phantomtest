...
import test.phantom.com.p90.entity.ReadBookContentBean;
...
import test.phantom.com.p90.ui.importbook.ImportBookModelImpl;
...
public class ReadBookPresenter implements ReadBookContract.Presenter {
    ...
    @Inject
    public ReadBookPresenter(ReadBookActivity view) {
        this.mView = view;
    }
        ...
        getRealFilePath(activity, uri)
                ...
                .subscribe(new SimpleObserver<String>() {
                        ...
                        ImportBookModelImpl.getInstance().importBook(new File(value))
                                ...
                                .subscribe(new SimpleObserver<LocBookShelfBean>() {
                                    ...
                                    public void onError(Throwable e) {
                                        ...
                                        Toast.makeText(BaseApplication.getInstance(), "文本打开失败！", Toast.LENGTH_SHORT)
                                                .show();
                                    }
                                });
                    }
                    ...
                    public void onError(Throwable e) {
                        ...
                        mView.dimissLoadBook();
                        mView.loadLocationBookError();
                        Toast.makeText(BaseApplication.getInstance(), "文本打开失败！", Toast.LENGTH_SHORT).show();
                    }
                });
    }
    ...
    public void initContent() {
        mView.initContentSuccess(bookShelf.getDurChapter(), bookShelf.getBookInfoBean().getChapterlist().size(),
                bookShelf.getDurChapterPage());
    }

    @Override
    public void loadContent(final BookContentView bookContentView, final long bookTag, final int chapterIndex, int
            pageIndex) {
        if (null != bookShelf && bookShelf.getBookInfoBean().getChapterlist().size() > 0) {
            if (null != bookShelf.getBookInfoBean().getChapterlist().get(chapterIndex).getBookContentBean() && null
                    != bookShelf.getBookInfoBean().getChapterlist().get(chapterIndex).getBookContentBean()
                    .getDurCapterContent()) {
                if (bookShelf.getBookInfoBean().getChapterlist().get(chapterIndex).getBookContentBean().getLineSize()
                        == mView.getPaint().getTextSize() && bookShelf.getBookInfoBean().getChapterlist().get
                        (chapterIndex).getBookContentBean().getLineContent().size() > 0) {
                    //已有数据
                    int tempCount = (int) Math.ceil(bookShelf.getBookInfoBean().getChapterlist().get(chapterIndex)
                    ...
                    int end = pageIndex == tempCount ? bookShelf.getBookInfoBean().getChapterlist().get(chapterIndex)
                            .getBookContentBean().getLineContent().size() : start + pageLineCount;
                    if (bookContentView != null && bookTag == bookContentView.getqTag()) {
                        bookContentView.updateData(bookTag, bookShelf.getBookInfoBean().getChapterlist().get
                                        (chapterIndex).getDurChapterName()
                                , bookShelf.getBookInfoBean().getChapterlist().get(chapterIndex).getBookContentBean()
                                        .getLineContent().subList(start, end)
                                , chapterIndex
                    ...
                    //有元数据  重新分行
                    bookShelf.getBookInfoBean().getChapterlist().get(chapterIndex).getBookContentBean().setLineSize
                            (mView.getPaint().getTextSize());
                    final int finalPageIndex = pageIndex;
                    SeparateParagraphtoLines(bookShelf.getBookInfoBean().getChapterlist().get(chapterIndex)
                            .getBookContentBean().getDurCapterContent())
                            .observeOn(AndroidSchedulers.mainThread())
                            ...
                            .subscribe(new SimpleObserver<List<String>>() {
                                ...
                                public void onNext(List<String> value) {
                                    bookShelf.getBookInfoBean().getChapterlist().get(chapterIndex).getBookContentBean
                                            ().getLineContent().clear();
                                    bookShelf.getBookInfoBean().getChapterlist().get(chapterIndex).getBookContentBean
                                            ().getLineContent().addAll(value);
                ...
                Observable.create(new ObservableOnSubscribe<ReadBookContentBean>() {
                    ...
                    public void subscribe(ObservableEmitter<ReadBookContentBean> e) throws Exception {
                        List<BookContentBean> tempList = DbHelper.getInstance().getmDaoSession()
                                .getBookContentBeanDao().queryBuilder().where(BookContentBeanDao.Properties
                                        .DurChapterUrl.eq(bookShelf.getBookInfoBean().getChapterlist().get
                                                (chapterIndex).getDurChapterUrl())).build().list();
                        e.onNext(new ReadBookContentBean(tempList == null ? new ArrayList<BookContentBean>() :
                                tempList, finalPageIndex1));
                        e.onComplete();
                    }
                }).observeOn(AndroidSchedulers.mainThread())
                        .subscribeOn(Schedulers.newThread())
                        .subscribe(new SimpleObserver<ReadBookContentBean>() {
                            ...
                            public void onNext(ReadBookContentBean tempList) {
                                if (tempList.getBookContentList() != null && tempList.getBookContentList().size() > 0
                                    ...
                                    bookShelf.getBookInfoBean().getChapterlist().get(chapterIndex).setBookContentBean
                                            (tempList.getBookContentList().get(0));
                                    loadContent(bookContentView, bookTag, chapterIndex, tempList.getPageIndex());
                                } else {
                                    final int finalPageIndex1 = tempList.getPageIndex();
                                    WebBookModel.Companion.getInstance().getBookContent(bookShelf.getBookInfoBean
                                                    ().getChapterlist().get(chapterIndex).getDurChapterUrl(),
                                            chapterIndex,
                                            bookShelf.getTag()).map(new Function<BookContentBean, BookContentBean>() {
                                        @Override
                                        public BookContentBean apply(BookContentBean bookContentBean) throws Exception {
                                            if (bookContentBean.getRight()) {
                                                DbHelper.getInstance().getmDaoSession().getBookContentBeanDao()
                                                        .insertOrReplace(bookContentBean);
                                                bookShelf.getBookInfoBean().getChapterlist().get(chapterIndex)
                                                        .setHasCache(true);
                                                DbHelper.getInstance().getmDaoSession().getChapterListBeanDao()
                                                        .update(bookShelf.getBookInfoBean().getChapterlist().get
                                                                (chapterIndex));
                                            }
                                            ...
                                            .subscribe(new SimpleObserver<BookContentBean>() {
                                                ...
                                                public void onNext(BookContentBean value) {
                                                    if (value.getDurChapterUrl() != null && value.getDurChapterUrl()
                                                            .length() > 0) {
                                                        bookShelf.getBookInfoBean().getChapterlist().get
                                                                (chapterIndex).setBookContentBean(value);
                                                        if (bookTag == bookContentView.getqTag()) {
                                                            loadContent(bookContentView, bookTag, chapterIndex,
                                                                    finalPageIndex1);
                                                        }
                                                    } else {
                                                        if (bookContentView != null && bookTag == bookContentView
                                                                .getqTag()) {
                                                            bookContentView.loadError();
                                                        }
                                                    }
    ...
    public Observable<List<String>> SeparateParagraphtoLines(final String paragraphstr) {
        return Observable.create(new ObservableOnSubscribe<List<String>>() {
            ...
            public void subscribe(ObservableEmitter<List<String>> e) throws Exception {
                ...
                Layout tempLayout = new StaticLayout(paragraphstr, mPaint, mView.getContentWidth(), Layout.Alignment
        ...
        Observable.create(new ObservableOnSubscribe<Boolean>() {
            ...
            public void subscribe(ObservableEmitter<Boolean> e) throws Exception {
                List<BookShelfBean> temp = DbHelper.getInstance().getmDaoSession().getBookShelfBeanDao().queryBuilder
                        ().where(BookShelfBeanDao.Properties.NoteUrl.eq(bookShelf.getNoteUrl())).build().list();
    ...
    public void addToShelf(final OnAddListner addListner) {
        if (bookShelf != null) {
            Observable.create(new ObservableOnSubscribe<Boolean>() {
                @Override
                public void subscribe(ObservableEmitter<Boolean> e) throws Exception {
                    DbHelper.getInstance().getmDaoSession().getChapterListBeanDao().insertOrReplaceInTx(bookShelf
                            .getBookInfoBean().getChapterlist());
                    DbHelper.getInstance().getmDaoSession().getBookInfoBeanDao().insertOrReplace(bookShelf
                            .getBookInfoBean());
            ...
            public void subscribe(ObservableEmitter<String> e) throws Exception {
                ...
                if (null != uri) {
                    ...
                    if (scheme == null) {
                        ...
                        if ((data == null || data.length() <= 0) && uri.getPath() != null && uri.getPath().contains
                                ("/storage/emulated/")) {
